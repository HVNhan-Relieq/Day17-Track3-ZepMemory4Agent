...
Cả bốn layer đều 100%. Layer tốn nhất là long-term: E03 1404 tok, E08 1396, E02 1395,
reduction 0% — Context Block trả nguyên khối summary cộng edge facts, và latency ~1.1 s
(prime thread + `get_user_context` + edge search). Semantic rẻ nhất: E11 146 tok / 74.2%,
E06 148 tok / 67.8%, ~0.2 s vì chỉ một lần `graph.search`.

E07 (mixed) ghép long-term + semantic; hai evidence bắt buộc là `Python` (user graph) và
`Idempotency-Key` (shared graph). `budget_breakdown`: long-term raw 1412 tok bị trim còn 324
(limit 320), semantic 148/240 nguyên vẹn. Vậy `Python` phải nằm ở **đầu** context block mới
sống sót — lý do `trim` giữ head chứ không giữ tail.
...
thắng preference Python *cho project đó*, nhưng không xóa preference ở scope khác — E02 vẫn
pass cùng lần chạy. Edge search `limit=20` kèm validity range giữ được cả hai fact.
...
**Zep Context Block vs Redis + Qdrant:** Zep tự trích fact, gắn validity và tổng hợp xuyên
thread; đổi lại ~1.1 s mỗi lần gọi và ingestion trễ bất định — seed của tôi phải nâng
`ZEP_POLL_TIMEOUT` lên 900 s mới qua stage 2. Redis + Qdrant latency gần 0 và kiểm soát hoàn
toàn, nhưng phải tự viết extraction, conflict resolution, deletion.
...
chứng minh right-to-be-forgotten (Zep user absent: True, Redis keys remaining: 0) trong khi
shared KB giữ nguyên vì chứa domain knowledge, không phải PII.
